...
- Sets `peak_hour: true/false` in the quota-status payload (proxy mode v3.5.0+: `~/.local/state/cache-fix/quota-status/account.json`; preload mode: `~/.local/state/cache-fix/quota-status.json`)
