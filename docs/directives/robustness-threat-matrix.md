...
| 6 | Tool-schema drift mid-session (ToolSearch loading deferred tools, MCP reconnect, schema bump) | tools[] change → front invalidation (tools render before system+messages; NO breakpoint can survive it) | **CONFIRMED LIVE** (2026-07-27 12:47:56, the 175k event: ledger row carries `tools[...SendMessage:added...]` + toolsMatch:false — a ToolSearch `select:SendMessage` load added the schema mid-session at message 165; the co-occurring user-injection was secondary). deferred-tools-restore did NOT absorb it. Mitigation is NOT pinning (the model genuinely needs the new schema — an honest content change): it is **timing**. Candidate: session-start PRELOAD list — tools that are near-certain to load eventually (SendMessage for any teammate-using session) cost ~0 to include at message 1 and ~context-size to load at message 165. Harness-side lever (eager list) preferred; proxy-side injection fallback. Secondary: when a deferred load must happen, it should ride a request that busts anyway (idle-return, post-compaction) — scheduling hint, harness-side. |
