...
| 15 | Subagent 5m TTL | CLOSED-already-shipped (STOP finding 2026-07-27: ttl-management.mjs defaults TTL_SUBAGENT="1h" via detectRequestType, issue #14, predates this row — row was written without cross-referencing the extension inventory; ttl-tier-detect carries the overage exception). Residual OPEN (narrow): subagent-scoped telemetry — cache-telemetry keys by session-id which subagent legs share with the parent, so per-leg fire-rate is uncountable; separate small ticket. Unbridged verification gap: no captured-traffic confirmation that subagent requests arrive without explicit ttl (snapshot format drops ttl fields) |
