...
minutes — and **`mtok` 0**. `"ttl":"1h"` appears 217 times on that
same session's own wire, so every entry had expired more than five
hours before the request was composed. `mtok` 0 is the discriminator
that makes this not-a-prefix-defect: no prefix matched AT ALL, against
267,780 on the genuine mid-history bust the same day. Transcript
diagnostic `previous_message_not_found`, i.e. CC's own words for the
same thing. Disposition: **CONTROLLED-CAUSE**, and the walk is
