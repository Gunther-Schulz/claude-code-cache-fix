...
WHAT IS NOT CLAIMED HERE. Whether any shipped mitigation FIRED and
failed to absorb on this pair is unanswered — `bust-triage` reports the
class, and the absorption question is `findAbsorptionMisses`', which
rides the daily sweep. The sweep started 2026-08-07T13:19Z with the
row-28 fix deployed and its rows for this capture are the evidence;
until they are read this line stays open rather than being guessed. No
