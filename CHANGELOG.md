...
- **BREAKING (paths): all proxy-written state moved out of `~/.claude/` into the XDG base directories.** `~/.claude/` is Claude Code's *config* root; this proxy's captures, CA, logs, ledgers and snapshots are *data*, and they only lived there by habit. Keeping them there also meant every read and write of a proxy artifact — by the operator and by any tooling — tripped the sensitive-path protection that keys on that directory's shape. Two new roots, split by "unrecoverable if lost → data; regenerable → state": **`$XDG_DATA_HOME/cache-fix`** (default `~/.local/share/cache-fix`) holds the capture corpus and the forward-proxy CA; **`$XDG_STATE_HOME/cache-fix`** (default `~/.local/state/cache-fix`) holds everything else — `quota-status/` and the flat `quota-status.json`, `usage.jsonl`, `usage-log/`, `session-mirrors/`, `snapshots/`, `state/`, `debug.log`, `gate-status.json`, `bootstrap-log.jsonl`, `keymap.jsonl`, `fire-ledger.jsonl`, `stats.json`, `upstream-baseline.json`, `upstream-changes.jsonl`, `workflow-derivation-events.jsonl`, `oauth-events.jsonl`, `cold-events.jsonl`, `anthropic-proxy-logs/`, and the default-off event logs `image-retry-events.jsonl`, `session-budget-events.jsonl`, `overage-warnings.jsonl`. The redundant `cache-fix-` filename prefix is dropped in the new roots (the directory already names the tool), so `~/.claude/cache-fix-debug.log` is now `~/.local/state/cache-fix/debug.log`. Every `CACHE_FIX_*` path override (`CACHE_FIX_CA_DIR`, `CACHE_FIX_USAGE_LOG`, `CACHE_FIX_SESSION_MIRROR_DIR`, `CACHE_FIX_QUOTA_STATUS_DIR`, …) still wins over the default, unchanged.

  **Migrate with `node tools/xdg-migrate.mjs --apply`.** It is a dry run by default (`node tools/xdg-migrate.mjs` prints what it would move), it refuses cross-device moves and refuses to merge into a non-empty destination rather than guessing, and `node tools/xdg-migrate.mjs --verify` reads every path back through the module that owns it so you can confirm each store arrived before restarting the proxy. Until you migrate, *readers* fall back to the old `~/.claude/` location once, with a `CacheFixLegacyPathWarning` on stderr naming the file; *writers* never fall back, so an unmigrated install will start writing a second store at the new path. That fallback is removed after one transition.

  **`CLAUDE_CONFIG_DIR` no longer relocates proxy state.** It now affects only the files Claude Code itself owns and the proxy merely reads — the shared OAuth credential (`.credentials.json`) and its lock. If you run one proxy per config root to keep their state apart, give each one its own `XDG_STATE_HOME`/`XDG_DATA_HOME` (or pin the individual `CACHE_FIX_*` overrides) instead. This narrows the behavior added in v4.3.0's "Honor `CLAUDE_CONFIG_DIR` for all on-disk proxy state".
### Fixed
