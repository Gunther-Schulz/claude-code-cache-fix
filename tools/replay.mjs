...
// The text-report rendering of findAbsorptionMisses, extracted so its
// content is testable without spawning the CLI over a full pipeline replay
// (BACKLOG "findAbsorptionMisses runs on every replay and prints on none").
// Printed unconditionally, zero included: the three numbers named are
// absorbed-at (where the absorption claimed to act), forwardedDivergence
// (where the forwarded pair actually diverged) and ours (whether CC's own
// input diverged there too) — the same three that turned the 349k bust from
// a puzzle into a mechanism (dev-loop.md, "What no gate asks: did the
// mitigation ABSORB?").
export function formatAbsorptionMisses(rows) {
  let out = `\nabsorption misses (the mitigation ran but the forwarded pair still diverged within its claimed reach): ${rows.length}\n`;
  for (const m of rows.slice(0, 20)) {
    out += `  n=${m.prevN}->${m.n} ts=${m.ts} absorbedAt=[${m.absorbedFreshAt.join(",")}]` +
      ` forwardedDivergence=${m.forwardedDivergence} ours=${m.ours}` +
      (m.cacheControlOnly === true ? " [cache_control marker only]" : "") +
      "\n";
  }
  return out;
}
...
async function main() {
  ...
  if (args.json) {
    ...
    // Always computed (findAbsorptionMisses runs on every replay, never
    // behind --census) — "now asks it on every run — not behind --census,
    // because the whole point is that nobody knew to look" (dev-loop.md).
    // Until this section existed the computation reached a human only via
    // --json or gate-live's status file; the text run a human actually
    // reads during a bust walk (docs/runbooks/bust-appears.md step 4) had
    // no absorption section at all, not even a zero line — the one real
    // miss on s-captureAM surfaced only because relocDepartures happened to
    // name the same pair by a different check. Printed unconditionally,
    // zero included, per the three-answer rule: a run that never mentions
    // absorption misses is indistinguishable from a run that found none.
    process.stdout.write(formatAbsorptionMisses(absorptionMisses));
