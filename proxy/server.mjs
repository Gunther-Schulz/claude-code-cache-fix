...
async function handleMessages(clientReq, clientRes) {
  ...
  // `sink` is this request's own response until a coalescing leader adopts
  // it; the close handler asks the SINK whether anyone is still listening,
  // so a leader whose own client hangs up keeps serving its followers
  // instead of aborting the upstream call they are waiting on.
  let sink = clientRes;
  clientReq.on("close", () => {
    if (!sink.writableEnded) abortController.abort();
  });
