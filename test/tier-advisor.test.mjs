...
function makeTempDir() {
  return tmpDirSync("tier-advisor-test-");
}
...
test("26. Statusline reads tier-advisor-state.json and appends tier:upgrade when last_recommendation is upgrade", () => {
  const dir = makeTempDir();
  try {
    // Write account.json (statusline requires it — it returns empty and exits
    // early without one, which is why this case saw '' rather than a missing
    // tier: token).
    //
    // FORK DEVIATION 2026-08-16 (upstream merge): upstream seeds this at
    // `$HOME/.claude/quota-status/account.json`, but this fork's statusline
    // resolves it from the XDG state root — `tools/quota-statusline.sh:29`
    // reads `${XDG_STATE_HOME:-$HOME/.local/state}/cache-fix/quota-status/`.
    // With HOME pointed at the temp dir and XDG_STATE_HOME unset, the script
    // looked under `<dir>/.local/state/...` and found nothing. The
    // tier-advisor-state.json path below is NOT changed: the script reads that
    // one from `$HOME/.claude` (:245) and `tools/tier-advisor.mjs` writes it
    // there too, so writer and reader already agree — that residual
    // inconsistency inside one script is booked, not silently repointed here.
    mkdirSync(join(dir, ".local", "state", "cache-fix", "quota-status"), { recursive: true });
    mkdirSync(join(dir, ".claude"), { recursive: true }); // still the tier-state home, per the note above
    writeFileSync(join(dir, ".local", "state", "cache-fix", "quota-status", "account.json"), JSON.stringify({
    ...
    // XDG_STATE_HOME is pinned, not merely inherited. The suite harness
    // (tools/suite-config-root.mjs) sets it to isolate the run, and it takes
    // precedence over HOME in the statusline's own resolution
    // (`${XDG_STATE_HOME:-$HOME/.local/state}`), so `{ ...process.env, HOME }`
    // alone left the script reading the harness's state root instead of this
    // case's. Verified by hand before fixing: the same seeded layout run with
    // HOME set and XDG_STATE_HOME unset emits the tier:upgrade token.
    const out = execFileSync("/bin/bash", [statusline], {
      env: { ...process.env, HOME: dir, XDG_STATE_HOME: join(dir, ".local", "state") },
      input: "{}",
      encoding: "utf8",
    });
