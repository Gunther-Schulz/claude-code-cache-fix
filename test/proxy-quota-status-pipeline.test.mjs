...
test("[pipeline #16] end-to-end happy path: full pipeline writes account.json + sessions/<uuid>.json", async () => {
  ...
  try {
    ...
    const sessionPath = join(env.home, ".local", "state", "cache-fix", "quota-status", "sessions", `${sid}.json`);
...
test("[pipeline #160] degraded path: no quota headers → no per-session write, but the high warn still fires once", async () => {
  ...
  try {
    ...
    assert.ok(!existsSync(join(env.home, ".local", "state", "cache-fix", "quota-status", "account.json")), "no account.json either");
...
test("[pipeline #11j] malformed session-id ends up in a hashed file, no path-traversal escape", async () => {
  ...
  try {
    ...
    const quotaDir = join(env.home, ".local", "state", "cache-fix", "quota-status");
