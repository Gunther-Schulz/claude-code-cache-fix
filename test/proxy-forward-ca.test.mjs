...
function scratchDir(prefix) {
  // Fork deviation from upstream's `mkdtempSync(join(tmpdir(), prefix))`: the
  // directory is allocated inside THIS process's run root (tools/tmpdir.mjs),
  // which the repo's no-raw-mkdtemp guard requires and which also covers the
  // abort paths after() never runs on. Upstream's registration + after() sweep
  // below are kept — they remove RSA key material promptly rather than at exit.
...
// FORK DEVIATION 2026-08-16 (upstream merge): upstream's per-file raw-mkdtemp
// guard stood here and has been REMOVED, not disabled, because this fork
// carries a strictly stronger one — `test/no-raw-mkdtemp.test.mjs` scans the
// whole of tools/, test/ and proxy/ and matches the NAME rather than the call
// shape, so it also catches the import-alias route (`import { mkdtemp as mkd }`)
// that a call-shape scan cannot see. Nothing is lost by dropping the local
// copy: this file is inside that guard's scan set.
//
// It could not simply be kept. That guard is deliberately CLOSED — no
// exemptions, because an exemption list is what the alias route defeats — and
// the assertion above named `mkdtempSync` inside its own pattern string, which
// the tree-wide guard correctly flags. Opening the strong guard to accommodate
// a redundant weak one is the wrong direction; the premise clause it also
// carried ("this file does create temp dirs") is preserved below.
test("ca-trust: this file really does mint temp dirs — the premise the tree-wide guard rests on", () => {
