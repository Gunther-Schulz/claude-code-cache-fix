...
import { chmodSync, closeSync, existsSync, fstatSync, mkdirSync, openSync, readFileSync, readdirSync, rmSync, statSync, utimesSync, writeFileSync } from "node:fs";
...
// Fork deviation from the upstream form: the directory is allocated inside
// THIS process's run root (tools/tmpdir.mjs) rather than bare `tmpdir()`, so
// the repo-wide leak discipline applies too — the run root is removed on exit,
// on throw and on SIGINT/SIGTERM/SIGHUP, which covers the abort paths after()
// never runs on. The central registration and after() sweep below are
// upstream's and are kept: they remove RSA key material promptly rather than
// at process exit, which is the property upstream measured this for.
const tempDirs = [];
function tempDir(prefix) {
  const d = tmpDirSync(prefix);
  tempDirs.push(d);
  return d;
}
after(() => {
...
function cleanEnv(overrides) {
  ...
  // Strip from the BASE, then apply overrides at the return, so a test that
  // deliberately sets one of these still gets it. An ambient NO_PROXY on the
  // developer's shell otherwise reaches the child and the merge assertions read
  // the host's value instead of the fixture's — the lowercase-no_proxy case
  // fails that way on any machine that exports NO_PROXY. This repo's own
  // wrapper exports BOTH NO_PROXY and no_proxy when claude runs through the
  // proxy, so running the suite from inside a wrapped session is exactly how
  // that ambient value arrives.
  ...
  // FORK DEVIATION 2026-08-16 (upstream merge). Upstream's cases read the MITM
  // CA at `<CLAUDE_CONFIG_DIR>/cache-fix-ca/ca.pem`, because upstream resolves
  // it from the config root. This fork resolves it from the XDG data root
  // (`config.caDir` → `dataPath("ca")`), so the config dir no longer places it
  // and fifteen of upstream's `--remote-control` cases read a path nothing
  // writes. Pinning the documented override here puts the CA back where those
  // cases look, in ONE line, rather than editing fifteen test bodies — and it
  // keeps a single resolver, which is the whole point of `proxy/xdg-dirs.mjs`:
  // this file's own history already records five proxy-wrapper tests failing
  // when a second, hand-rolled resolution went stale against it.
  //
  // Derived AFTER the override spread, not before. Most cases here pass their
  // own `CLAUDE_CONFIG_DIR` as an override, so computing this from the base
  // env pointed the CA dir at the default `cffcfg-` temp while the client used
  // the case's own `cfftrust-` one — the same value in two coordinate spaces,
  // which reads as "the proxy never wrote a CA".
  if (!merged.CACHE_FIX_CA_DIR) {
    merged.CACHE_FIX_CA_DIR = join(merged.CLAUDE_CONFIG_DIR, "cache-fix-ca");
  }
  return merged;
}
...
describe("launch wrapper (claude-via-proxy)", () => {
  ...
  it("--remote-control wires forward-proxy env (BASE unset, HTTPS_PROXY + CA set)", async () => {
    ...
    // The directory NAME here follows whatever cleanEnv pinned as
    // CACHE_FIX_CA_DIR, not the XDG default. Changed 2026-08-16 (upstream
    // merge): this used to assert `cache-fix/ca/ca.pem`, the fork's
    // `dataPath("ca")` default, while the 28 upstream cases merged alongside it
    // read `<config>/cache-fix-ca`. Pinning the override in cleanEnv serves 28
    // sites against this one, and nothing is lost — the fork's default
    // resolution is still asserted end to end by the very next case, which
    // drives `config.caDir` itself. What THIS case is for is the three routing
    // vars, and that is unchanged: BASE unset, HTTPS_PROXY set, and
    // NODE_EXTRA_CA_CERTS pointing at the CA the spawned proxy actually made.
  ...
  it("creates every temp dir through the registrar, so none outlive the run", () => {
    ...
    // FORK DEVIATION 2026-08-16 (upstream merge): upstream's source-level scan
    // stood here and has been REMOVED, not disabled — `test/no-raw-mkdtemp.mjs`
    // covers this file already, scans the whole tree, and matches the NAME
    // rather than the call shape, so it also catches the import-alias route a
    // call-shape scan cannot see. That guard is deliberately closed to
    // exemptions, and the assertion here named `mkdtempSync` inside its own
    // pattern string, which the tree-wide guard correctly flagged. The premise
    // clause is what remains, since it is the half the tree-wide guard cannot
    // assert for this file.
