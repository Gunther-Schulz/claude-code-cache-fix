...
## Closed 2026-09-21 — the conversation-keyed prefix-diff baseline shipped

Both bodies MOVED here verbatim at closure, per the `Closure-home:` declaration
in BACKLOG.md. Shipped as cache-fix `bfc5079` (change), `96795e8` (carrier
residual closed at the consumers' own tap), `cc12325` (a duplicated sentence in
my own edit), with the dotfiles tree pin at `fd5cbf2` (`5ddf24f` -> `52442e4`).
Proxy restarted and verified serving: `/health`'s `proxy_tree` equals a fresh
content fingerprint of the tree on disk.

What closed them: desk-to-desk diffs readable 0 of 9 before and 9 of 9 after;
49 shrink diffs at HEAD all unlabelled becoming 39 all labelled; the threat
matrix exemption lifted on two grounds with an explicit refusal of a third.
Graded by two fresh-context rounds plus a scoped delta grade, each naming the
body it held by hash.

What did NOT close, and is deliberately still live in BACKLOG.md: the row-4
instance data (the bust MECHANISM is unexplained — two candidate causes refuted
with controls, the survivor unproven), and the invariant-guard entry (the guard
that was promised to fail loudly still does not; with the exemption now lifted
its job has CHANGED rather than gone, so it wants an assertion that prefix-diff
now DOES key on the conversation sub-key).
